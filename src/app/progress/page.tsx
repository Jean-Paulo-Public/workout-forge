...
export default function ProgressTrackingPage() {
  ...
  return (
    <AppLayout>
      ...
      {sessionToLogWeights && workoutToLogWeights && (
        <LogWeightsModal
          ...
          workout={workoutToLogWeights} // workout for the session
...
// Helper to get workout details for the modal, should be defined or imported
// For now, it's a local helper. Could be moved to context if needed more broadly.
function useWorkoutForSession(sessionId: string | null): Workout | undefined {
  const { sessions, getWorkoutById } = useAppContext();
  if (!sessionId) return undefined;
  const session = sessions.find(s => s.id === sessionId);
  if (!session) return undefined;
  return getWorkoutById(session.workoutId);
}

// We need to pass workout to LogWeightsModal, so we need to get it.
// Adjusting the main component:
export default function ProgressTrackingPageWithModalWorkout() {
